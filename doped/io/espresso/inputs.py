...
from doped.generation import _custom_formatwarning
...
# aliases used by the competing-phase input generation functions below (moved from ``outputs.py``):
_qe_convergence_defaults = default_qe_set
_qe_hse_relax_defaults = default_qe_HSE_set
...
:class:`DefectsSet`, in decreasing order of accuracy -- and so the priority
...
def _build_qe_base_settings(
    ...
    # TODO: Input files for multi-oxidation states (as in ``doped/qe.py``); ``ntyp`` here counts
    # distinct ``Species`` while ``write_qe_pw_input`` strips oxidation states and writes one
    # ``ATOMIC_SPECIES`` line per element, so a structure with multiple oxidation states of the same
    # element (e.g. Fe(2+)/Fe(3+)) gives ``ntyp`` greater than the number of written species, which
    # espresso rejects.
    base["system"]["ntyp"] = len(set(structure.species))

    if is_metal:
        _set_default_metal_smearing(base["system"])
...
class DopedDictSetQE(MSONable):
    ...
    # ``pseudopotentials`` property is re-evaluated for every defect in a ``DefectsSet``
    ...
    def write_input(self, output_dir: PathLike, filename: str = "pw.in") -> None:
        """
        Write the ``pw.x`` input file for this input set to
        ``<output_dir>/<filename>`` (creating ``output_dir`` if needed); the
        espresso analogue of ``VaspInputSet.write_input()``.

        Args:
            output_dir (PathLike):
                Directory to write the input file to.
            filename (str):
                Name of the ``pw.x`` input file. Default is ``"pw.in"``.
        """
        self.write_qe_pw_input(
            filepath=os.path.join(str(output_dir), filename),
            structure=self.structure,
            namelist_settings=self.namelists,
    ...
    def write_all(
        ...
        Args:
            ...
            bulk (bool, str):
                ...
                ``espresso_std``) -- plus ``espresso_hybrid`` if
                ``hybrid=True``, as hybrid DFT is the highest accuracy of all.
            bulk_dir (PathLike):
        ...
        **kwargs,
        ...
        Args:
            ...
            bulk (bool, str):
                ...
                ``soc``, otherwise ``espresso_std``) -- plus
                ``espresso_hybrid`` if ``hybrid=True``, as hybrid DFT is the
                highest accuracy of all. If ``bulk = "all"``, it is
...
def qe_convergence_setup_from_structure(
    ...
    After running these and choosing converged values, feed them to
    :func:`get_relaxation_sets` / :func:`write_relaxation_files` -- the cutoffs
    via ``user_system_settings`` (e.g. ``{"ecutwfc": 80, "ecutrho": 320}``) and
    the k-point densities via ``kpoints_metals``/``kpoints_nonmetals`` -- to
    write the final ``vc-relax`` calculations of the competing phases.
    ...
    Args:
        ...
        is_metal: If ``True``, set ``occupations='smearing'``,
            ``smearing='gaussian'``, ``degauss=0.005`` in ``&SYSTEM`` (unless
            ``occupations`` is given in ``user_system_settings``).
...
def _default_qe_competing_phases_path(composition: str | Composition) -> str:
    ...
    Used as the default ``output_path`` for :func:`get_kpoint_convergence_sets`
    / :func:`get_relaxation_sets` / :func:`get_singlepoint_sets` (and the
    corresponding ``write_*_files`` functions), so that generated inputs can be
    found again (by passing the same path to
...
def _competing_phase_system_settings(
    entry: ComputedEntry | ComputedStructureEntry, user_system_settings: dict | None, soc: bool
) -> dict:
    """
    ``&SYSTEM`` namelist overrides for one competing phase: the user settings,
    plus spin polarisation for magnetic phases (via
    :func:`_set_spin_polarisation`, which seeds ``starting_magnetization``
    rather than ``nspin``/``tot_magnetization`` for noncolinear (``soc``)
    calculations, hence the ``noncolin``/``lspinorb`` flags are set here too).
    """
    system_settings = copy.deepcopy(user_system_settings or {})
    if soc:  # spin-orbit coupling: noncolinear magnetisation with fully-relativistic pseudos
        system_settings.setdefault("noncolin", True)
        system_settings.setdefault("lspinorb", True)
    _set_spin_polarisation(system_settings, user_system_settings or {}, entry)
...
def get_kpoint_convergence_sets(
    ...
    user_control_settings: dict | None = None,
    user_electron_settings: dict | None = None,
) -> dict[str, DopedDictSetQE]:
    r"""
    Generates a dictionary of :class:`DopedDictSetQE`\s for `k`-point `and`
    plane-wave cutoff (``ecutwfc``) convergence testing of competing phases,
    as single-point (``scf``) ``pw.x`` calculations using the
    :data:`QE_PSEUDO_LIBRARY` pseudopotential filenames by default; the
    espresso analogue of
    :func:`doped.io.vasp.inputs.get_kpoint_convergence_sets`.

    Unlike with VASP, an ``ecutwfc`` convergence sweep is also generated
    here, as the required plane-wave cutoff depends on the pseudopotentials
    used. Two sub-trees are generated per phase (under
    ``<output_path>/<phase>/``):

    - ``kpoint_converge/k<kx>_<ky>_<kz>``: `k`-point grid varied at fixed
      ``ecutwfc``. Skipped for diatomic molecules (Γ-only sampling is exact
      for molecules-in-a-box, so there is no `k`-point grid to converge).
    - ``ecut_convergence/ecutwfc_<N>``: ``ecutwfc`` varied at a fixed
      `k`-point grid. Generated for `all` phases including molecules (the
      plane-wave cutoff must be converged for molecules too). ``ecutrho`` is
      set to ``4 * ecutwfc`` at each step (i.e. a fixed 'dual'), unless
      ``ecutrho`` is given in ``user_system_settings``.

    Automatically sets Gaussian smearing (``occupations = 'smearing'``) for
    metallic phases, and spin polarisation (``nspin = 2`` and
    ``tot_magnetization``, or ``starting_magnetization`` if ``soc``) for
    magnetic phases.
    ...
    Args:
        ...
        kpoint_sweep_ecutwfc (int or None):
            ``ecutwfc`` (Ry) held fixed while the k-grid is swept (with
            ``ecutrho`` then set to ``4 * ecutwfc``, unless given in
            ``user_system_settings``). ``None`` (default) keeps the YAML set
            defaults (60 Ry, ``ecutrho`` = 240 Ry).
    ...
    ecut_kppvol_by_metallicity = {"non-metals": ecut_kpoints_nonmetals, "metals": ecut_kpoints_metals}
    ecut_min, ecut_max, ecut_step = ecut_convergence
    dict_sets: dict[str, DopedDictSetQE] = {}

    for entry, category, structure, folder_name in competing_phases._iter_entries_with_categories():
        phase_folder = os.path.join(str(output_path), folder_name)
        dict_set_kwargs = {  # settings shared by all convergence calculations of this phase
            "calculation": "scf",  # convergence testing uses single-point SCF
            "kpoints_shift": kpoints_shift,
            "pseudo_dir": pseudo_dir,
            "pseudo_map": pseudo_map,
            "is_metal": category == "metals",  # only metallic phases get Gaussian smearing
            "soc": soc,
            "user_control_settings": user_control_settings,
            "user_system_settings": _competing_phase_system_settings(entry, user_system_settings, soc),
            "user_electron_settings": user_electron_settings,
        }

        # k-point convergence: vary k-grid at fixed ecutwfc. Skipped for molecules (Γ-only is exact
        # for a molecule-in-a-box).
        if category != "molecules":
            min_k, max_k, step_k = kpoints_by_metallicity[category]
            seen_kgrids: set[tuple[int, ...]] = set()
            for kpoint in np.arange(min_k, max_k, step_k):
                kgrid = _kpoints_grid_from_reciprocal_density(structure, kpoint)
                if tuple(kgrid) in seen_kgrids:
                    continue  # this k-point grid already generated, bump reciprocal density more
                seen_kgrids.add(tuple(kgrid))
                kname = "k" + "_".join(str(k) for k in kgrid)
                dict_sets[os.path.join(phase_folder, "kpoint_converge", kname)] = DopedDictSetQE(
                    structure, ecutwfc=kpoint_sweep_ecutwfc, kpoints=kgrid, **dict_set_kwargs
                )

        # ecutwfc convergence: vary ecutwfc at a fixed k-grid (Γ-only for molecules)
        ecut_kgrid = (
            None
            if category == "molecules"
            else _kpoints_grid_from_reciprocal_density(structure, ecut_kppvol_by_metallicity[category])
        )
        for ecut in range(ecut_min, ecut_max + 1, ecut_step):
            dict_sets[os.path.join(phase_folder, "ecut_convergence", f"ecutwfc_{ecut}")] = (
                DopedDictSetQE(structure, ecutwfc=ecut, kpoints=ecut_kgrid, **dict_set_kwargs)
            )
...
def write_input_sets(input_sets: dict[str, DopedDictSetQE], **kwargs) -> dict[str, DopedDictSetQE]:
    r"""
    Write a dictionary of :class:`DopedDictSetQE`\s (``{output folder: input
    set}``, as generated by :func:`get_kpoint_convergence_sets` /
    :func:`get_relaxation_sets` / :func:`get_singlepoint_sets`) to their
    corresponding output folders (as ``<output folder>/pw.in``), warning if
    any already exist; the espresso analogue of
    :func:`doped.io.vasp.inputs.write_input_sets`.

    Args:
        input_sets (dict[str, DopedDictSetQE]):
            Mapping of output folder paths to :class:`DopedDictSetQE`\s.
        **kwargs:
            Additional keyword arguments for
            :meth:`DopedDictSetQE.write_input` (e.g. ``filename``).

    Returns:
        dict[str, DopedDictSetQE]:
            The written ``input_sets``.
    """
    for folder, input_set in input_sets.items():
        if os.path.exists(folder):
            warnings.warn(f"Output folder {folder} already exists. Overwriting files.")
        input_set.write_input(folder, **kwargs)
...
def write_kpoint_convergence_files(
    ...
    **kwargs,
    ...
    r"""
    Generates and writes ``pw.x`` input files for `k`-point `and` plane-wave
    cutoff (``ecutwfc``) convergence testing of competing phases (as
    single-point ``scf`` calculations), to
    ``<output_path>/<phase>/kpoint_converge/k<kx>_<ky>_<kz>/pw.in`` and
    ``<output_path>/<phase>/ecut_convergence/ecutwfc_<N>/pw.in``; the
    espresso analogue of
    :func:`doped.io.vasp.inputs.write_kpoint_convergence_files` (formerly
    ``CompetingPhasesQE.qe_convergence_setup``), which additionally sweeps
    ``ecutwfc`` as the required cutoff depends on the pseudopotentials used.

    The input sets are generated with :func:`get_kpoint_convergence_sets`
    (see there for details) and written with :func:`write_input_sets`,
    returning the corresponding dictionary of :class:`DopedDictSetQE`\s.
    ...
    Args:
        ...
        user_electron_settings (dict or None):
            Override default ``&ELECTRONS`` namelist settings.
        **kwargs:
            Additional keyword arguments for :func:`write_input_sets` /
            :meth:`DopedDictSetQE.write_input` (e.g. ``filename``).

    Returns:
        dict[str, DopedDictSetQE]:
            Mapping of output folder paths to the written
            :class:`DopedDictSetQE`\s.
    """
    input_sets = get_kpoint_convergence_sets(
        ...
        kpoint_sweep_ecutwfc=kpoint_sweep_ecutwfc,
        ecut_convergence=ecut_convergence,
        ecut_kpoints_metals=ecut_kpoints_metals,
        ecut_kpoints_nonmetals=ecut_kpoints_nonmetals,
        pseudo_dir=pseudo_dir,
        pseudo_map=pseudo_map,
        soc=soc,
        kpoints_shift=kpoints_shift,
...
def get_relaxation_sets(
    ...
    user_ions_settings: dict | None = None,
    user_cell_settings: dict | None = None,
    subfolder: PathLike = "espresso_std",
) -> dict[str, DopedDictSetQE]:
    r"""
    Generates a dictionary of :class:`DopedDictSetQE`\s for standard
    (variable-cell) ``pw.x`` relaxations of competing phases, using the
    default settings by default (i.e. the functional implied by your chosen
    pseudopotentials), or HSE06 hybrid DFT if ``use_hse=True``; the espresso
    analogue of :func:`doped.io.vasp.inputs.get_relaxation_sets` -- note
    that the VASP function uses HSE06 by default, while here the semi-local
    (GGA) defaults are used.

    Solid phases are set up as variable-cell (``vc-relax``) relaxations,
    while diatomic molecules (molecules-in-a-box) are relaxed at fixed cell
    (``relax``) with Γ-only sampling. If ``calculation`` is given in
    ``user_control_settings`` it is used instead (as done by
    :func:`get_singlepoint_sets` for ``scf`` calculations), with
    variable-cell modes still replaced by ``relax`` for molecules.

    Automatically sets tetrahedron-method occupations
    (``occupations = 'tetrahedra'``) for `metallic` phases only. Override via
    ``user_system_settings``.

    Each phase input set is keyed by its output folder,
    ``<output_path>/<phase>/<subfolder>`` (``espresso_std`` by default), i.e.
    under a single host-compound parent folder (the layout expected by
    :class:`~doped.chemical_potentials.CompetingPhasesAnalyzer` with
    ``calculator="espresso"``).

    Note that this function uses a single k-point density setting each for
    metals (``kpoints_metals``), non-metals (``kpoints_nonmetals``) and
    molecules (Γ-only), while one will often want to specify custom k-point
    settings for each material individually based on convergence testing
    (e.g. using :func:`get_kpoint_convergence_sets`) to minimise cost.
    ...
    Args:
        ...
        subfolder (PathLike):
            ...
            ``<output_path>/<competing_phase_dir>/<subfolder>``. Default is
            ``"espresso_std"``. Set to ``"."`` to write input files directly
            to ``<output_path>/<competing_phase_dir>``, with no subfolders
            created.
    ...
    # std setup does full cell relaxation, unless the user overrides the ``calculation`` mode (e.g.
    # ``scf`` for the single-point sets of ``get_singlepoint_sets``). Molecules-in-a-box are always
    # fixed-cell (the box is not physical), so any variable-cell mode is replaced by ``relax`` for them:
    user_calculation = (user_control_settings or {}).get("calculation")
    solid_calculation = user_calculation or "vc-relax"
    molecule_calculation = (
        "relax" if user_calculation is None or user_calculation.startswith("vc-") else user_calculation
    )
    kpoints_by_metallicity = {"non-metals": kpoints_nonmetals, "metals": kpoints_metals}
    dict_sets: dict[str, DopedDictSetQE] = {}

    for entry, category, structure, folder_name in competing_phases._iter_entries_with_categories():
        system_settings = _competing_phase_system_settings(entry, user_system_settings, soc)
        if category == "molecules":  # Γ-only sampling for molecules-in-a-box
            calculation, kgrid = molecule_calculation, None
        else:
            calculation = solid_calculation
            kgrid = _kpoints_grid_from_reciprocal_density(structure, kpoints_by_metallicity[category])
            if category == "metals":  # tetrahedron method (no smearing) for metals; user setting wins
                system_settings.setdefault("occupations", "tetrahedra")

        dict_sets[os.path.join(str(output_path), folder_name, str(subfolder))] = DopedDictSetQE(
            structure,
            calculation=calculation,
            kpoints=kgrid,
            kpoints_shift=kpoints_shift,
            pseudo_dir=pseudo_dir,
            pseudo_map=pseudo_map,
            is_metal=category == "metals",
            use_hse=use_hse,
            soc=soc,
            user_control_settings=user_control_settings,
            user_system_settings=system_settings,
            user_electron_settings=user_electron_settings,
...
def write_relaxation_files(
    ...
    **kwargs,
    ...
    r"""
    Generates and writes espresso ``pw.in`` input files for standard
    (variable-cell) relaxations of competing phases, to
    ``<output_path>/<phase>/<subfolder>/pw.in`` (``espresso_std`` subfolder
    by default), using the default settings
    by default (i.e. the functional implied by your chosen pseudopotentials),
    or HSE06 hybrid DFT if ``use_hse=True``. Note that the VASP function uses
    HSE06 by default, while here the semi-local (GGA) defaults are used.

    The input sets are generated with :func:`get_relaxation_sets` (see there
    for details) and written with :func:`write_input_sets`, returning the
    corresponding dictionary of :class:`DopedDictSetQE`\s.
...
def get_singlepoint_sets(
    ...
    r"""
    Generates a dictionary of :class:`DopedDictSetQE`\s for single-point
    (``scf``) energy calculations of the competing phases, using the default
    settings by default (i.e. the functional implied by your chosen
    pseudopotentials), or HSE06 hybrid DFT if ``use_hse=True``; the espresso
    analogue of :func:`doped.io.vasp.inputs.get_singlepoint_sets` -- note
    that the VASP function uses HSE06 by default, while here the semi-local
    (GGA) defaults are used.

    These are expected to be used as the final energy calculation after
    geometry relaxation (from :func:`write_relaxation_files`), to obtain
    accurate total energies with tight convergence settings (and settings
    consistent with the defect supercell calculations). All phases
    (including molecules-in-a-box, with Γ-only sampling) are set up as
    ``scf`` calculations, generated via :func:`get_relaxation_sets` with the
    ``calculation`` mode overridden -- so the same k-point density,
    pseudopotential, spin-polarisation and metal-occupations handling
    applies (see there).

    If ``soc=True``, spin-orbit coupling (SOC) is included (by setting
    ``noncolin`` and ``lspinorb`` in ``&SYSTEM``) and the output subfolder
    name will default to ``"espresso_ncl"`` (if not set otherwise). If
    ``soc`` is not explicitly set (i.e. is ``None``), it defaults to
    ``True`` for systems where the max atomic number across all species
    (host and extrinsic) is Z >= 31 (heavier than Zn), matching the
    convention in :func:`doped.io.vasp.inputs.get_singlepoint_sets` and
    ``DefectsSet``. Note that SOC calculations in espresso require
    fully-relativistic pseudopotentials, so ``pseudo_map`` should then point
    at these (the :data:`QE_PSEUDO_LIBRARY` defaults are
    scalar-relativistic).

    For metals, the tetrahedron method (``occupations = 'tetrahedra'``) is
    used by default (as in :func:`get_relaxation_sets`; override via
    ``user_system_settings``).
    ...
    Each phase input set is keyed by its output folder,
    ``<output_path>/<phase>/<subfolder>``, i.e. under a single host-compound
    parent folder.
    ...
    Args:
        ...
        pseudo_map (dict or None):
            ``{element_symbol: pseudo_filename}`` mapping. Defaults to the
            :data:`~doped.io.espresso.inputs.QE_PSEUDO_LIBRARY` filename
            for each element; any entries given here override that default
            per element. Required to point at fully-relativistic
            pseudopotentials if ``soc`` is ``True``.
        use_hse (bool):
            If ``True``, use HSE06 hybrid DFT settings. If ``False``
            (default), use the default settings. Note that any changes to
            the functional should be consistent with those used for the
        ...
        soc (bool or None):
            ...
            ``noncolin=.true.`` and ``lspinorb=.true.`` in ``&SYSTEM``
            for every phase (requires fully-relativistic pseudopotentials;
            see ``pseudo_map``). If not set (``soc = None``; default), SOC
            is enabled when the max atomic number across all species (host
            and extrinsic) is Z >= 31. Magnetic phases are then seeded via
            ``starting_magnetization`` instead of
            ``nspin``/``tot_magnetization`` (which espresso does not allow
            for noncolinear calculations), and the default ``subfolder``
            name is set to ``"espresso_ncl"``. Should be consistent with
            the defect supercell calculations.
        kpoints_shift (tuple):
            ``(sx, sy, sz)`` grid offset (each 0 or 1) for the
            ``K_POINTS automatic`` card, e.g. ``(1, 1, 1)`` for a
            half-grid (Γ-shifted) Monkhorst-Pack mesh. Default
        ...
        user_system_settings (dict or None):
            Override default ``&SYSTEM`` namelist settings.
        user_control_settings (dict or None):
            Override default ``&CONTROL`` namelist settings (take
            precedence over the ``calculation = 'scf'`` setting here).
        user_electron_settings (dict or None):
            Override default ``&ELECTRONS`` namelist settings (e.g. a
            tighter ``{"conv_thr": 1e-10}``).
        subfolder (PathLike or None):
            Output folder structure is
            ``<output_path>/<competing_phase_dir>/<subfolder>`` where
            ``subfolder`` = ``"espresso_gamma"`` by default if ``soc`` is
            ``False``, or ``"espresso_ncl"`` if ``soc`` is ``True``. Set to
    ...
    if soc is None:
        ...
        soc = max(Element(el).Z for el in all_elements) >= 31
        ...
        if soc:  # if SOC being automatically determined, print an info message
            print(
                ...
                "calculations, as the heaviest element present (across intrinsic and extrinsic species) "
                "has an atomic number Z >= 31"
            )

    if subfolder is None:
        subfolder = "espresso_ncl" if soc else "espresso_gamma"

    # single-point (``scf``) calculations for all phases; user ``&CONTROL`` settings take precedence:
    sp_control_settings = {"calculation": "scf", **(user_control_settings or {})}

    # reuse the relaxation set generation with the single-point ``calculation`` override
    return get_relaxation_sets(
        ...
        kpoints_shift=kpoints_shift,
        user_system_settings=user_system_settings,
        user_control_settings=sp_control_settings,
        user_electron_settings=user_electron_settings,
        subfolder=subfolder,
    )
...
def write_singlepoint_files(
    ...
    **kwargs,
    ...
    r"""
    Generates and writes espresso ``pw.in`` input files for single-point
    (``scf``) energy calculations of the competing phases, to
    ``<output_path>/<phase>/<subfolder>/pw.in`` (``espresso_gamma`` subfolder
    by default, or ``espresso_ncl`` if ``soc``), using the default settings
    by default (i.e. the functional implied by your chosen pseudopotentials),
    or HSE06 hybrid DFT if ``use_hse=True``; the espresso analogue of
    :func:`doped.io.vasp.inputs.write_singlepoint_files` -- note that the
    VASP function uses HSE06 by default, while here the semi-local (GGA)
    defaults are used.

    If ``soc`` is not explicitly set, SOC is enabled when the max atomic number
    across all species (host and extrinsic) is Z >= 31 (requiring fully-relativistic
    pseudopotentials via ``pseudo_map``).

    The input sets are generated with :func:`get_singlepoint_sets` (see
    there for details) and written with :func:`write_input_sets`, returning
    the corresponding dictionary of :class:`DopedDictSetQE`\s.
    ...
    Args:
        ...
        soc (bool or None):
            ...
            see ``pseudo_map``). If not set (``soc = None``; default), SOC
            is enabled when the max atomic number across all species (host
            and extrinsic) is Z >= 31, and the default ``subfolder`` name
            is then set to ``"espresso_ncl"``. Should be consistent with
            the defect supercell calculations.
        kpoints_shift (tuple):
            ``(sx, sy, sz)`` grid offset (each 0 or 1) for the
            ``K_POINTS automatic`` card, e.g. ``(1, 1, 1)`` for a
...
def _set_spin_polarisation(
    ...
    namelist if the entry has a total magnetization greater than the default
    tolerance of 0.1.
...
def _set_default_metal_smearing(system_settings: dict) -> None:
    """
    Set ``occupations = 'smearing'`` (if ``occupations`` is not already set),
    with ``smearing = 'gaussian'`` and ``degauss = 0.005`` Ry, in the espresso
    ``&SYSTEM`` namelist for metallic phases.  Analogous to
    ``doped.io.vasp.inputs._set_default_metal_smearing`` for VASP (``ISMEAR``
    / ``SIGMA``).

    If ``occupations`` is already set to anything other than ``'smearing'``
    (e.g. ``'tetrahedra'``, as used for metals by :func:`get_relaxation_sets`
    / :func:`get_singlepoint_sets`), no smearing parameters are added.
    """
    system_settings.setdefault("occupations", "smearing")
    if str(system_settings["occupations"]).lower() == "smearing":
